...
contract ScaleOrders is Test, Deployers {
...
function test_validateAndPrepareScaleOrders() public {
    int24 tickSpacing = poolKey.tickSpacing;
    ILimitOrderManager.OrderInfo[] memory orders;
    vm.expectRevert("Rounded top tick must be above rounded bottom tick for token0 orders");
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 50, 0, true, 2, 1e18, tickSpacing);
    vm.expectRevert("Rounded top tick must be above rounded bottom tick for token0 orders");
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 70, 0, true, 2, 1e18, tickSpacing);

    vm.expectRevert("Rounded bottom tick must be below rounded top tick for token1 orders");
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 50, 60, false, 2, 1e18, tickSpacing);
    vm.expectRevert("Rounded bottom tick must be below rounded top tick for token1 orders");
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 70, 80, false, 2, 1e18, tickSpacing);

    vm.expectRevert(TickLibrary.MaxOrdersExceeded.selector);
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 130, 0, true, 2, 1e18, tickSpacing);
    vm.expectRevert(TickLibrary.MaxOrdersExceeded.selector);
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 130, 160, false, 2, 1e18, tickSpacing);
    ...
    // =========== Create tight orders in Token ===========
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 60, -3000, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 0);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 90, -3000, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 0);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 120, -3000, true, 3, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 0); assertEq(orders[2].bottomTick, 60);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 150, -3000, true, 3, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 0); assertEq(orders[2].bottomTick, 60);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 180, -3000, true, 4, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 0);
    assertEq(orders[2].bottomTick, 60); assertEq(orders[3].bottomTick, 120);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 210, -3000, true, 4, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 0);
    assertEq(orders[2].bottomTick, 60); assertEq(orders[3].bottomTick, 120);

    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 60, 3000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 0);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 90, 3000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 0);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 120, 3000, false, 3, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 0); assertEq(orders[2].bottomTick, 60);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 150, 3000, false, 3, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 0); assertEq(orders[2].bottomTick, 60);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 180, 3000, false, 4, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 0);
    assertEq(orders[2].bottomTick, 60); assertEq(orders[3].bottomTick, 120);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 210, 3000, false, 4, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 0);
    assertEq(orders[2].bottomTick, 60); assertEq(orders[3].bottomTick, 120);
    
  
    // =========== Orders in Token Zero ===========
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 180, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 120);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 210, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 120);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 240, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 180);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 270, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 180);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 300, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 240);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 330, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 240);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 360, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 300);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 3600, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 3540);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);

    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 180, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 120);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 210, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 120);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 240, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 180);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 270, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 180);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 300, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 240);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 330, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 240);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 360, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 300);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 3600, -100, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 3540);
    
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 180, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 120);
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 210, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 120);
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 240, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 180);
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 270, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 180);
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 300, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 240);
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 330, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 240);
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 360, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 300);
    orders = TickLibrary.validateAndPrepareScaleOrders(10, 3600, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 3540);

    orders = TickLibrary.validateAndPrepareScaleOrders(60, 180, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 120);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(60, 210, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 120);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(60, 240, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 180);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(60, 270, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 180);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(60, 300, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 240);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(60, 330, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 240);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(60, 360, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 300);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(60, 3600, 0, true, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 60); assertEq(orders[1].bottomTick, 3540);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);

    // =========== Orders in Token One ===========
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 180, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 120);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 210, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 120);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 240, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 180);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 270, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 180);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 300, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 240);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 330, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 240);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 360, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 300);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);
    orders = TickLibrary.validateAndPrepareScaleOrders(-60, 3600, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, -60); assertEq(orders[1].bottomTick, 3540);
    assertEq(orders[0].topTick, orders[0].bottomTick + tickSpacing);
    assertEq(orders[1].topTick, orders[1].bottomTick + tickSpacing);

    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 180, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 120);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 210, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 120);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 240, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 180);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 270, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 180);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 300, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 240);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 330, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 240);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 360, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 300);
    orders = TickLibrary.validateAndPrepareScaleOrders(-10, 3600, 4000, false, 2, 1e18, tickSpacing);
    assertEq(orders[0].bottomTick, 0); assertEq(orders[1].bottomTick, 3540);
    ...
    // Uncomment if you need logging for debugging
    // for(uint256 i = 0; i < orders.length; i++) {
    //     console.log("Order ID: ", i);
    //     console.log("Bottom: ", orders[i].bottomTick);
    //     console.log("Top: ", orders[i].topTick);
    // }
}
    function test_create_scale_orders() public {
        ...
        ILimitOrderManager.CreateOrderResult[] memory results = limitOrderManager.createScaleOrders(false, -60, 60, 3 ether, 2, 2e18, poolKey);
...
function test_scenarios_validateAndPrepareScaleOrders() public {
        // Provide tokens for tests
        deal(Currency.unwrap(currency0), address(this), 1000 ether);
        deal(Currency.unwrap(currency1), address(this), 1000 ether);
    ...
    function getLeftoverPositions(PoolId poolId) internal view returns (ILimitOrderManager.PositionTickRange[] memory) {
        ...
        ILimitOrderManager.PositionInfo[] memory positions = limitOrderManager.getUserPositions(address(this), poolId);
        uint256 positionsLength = positions.length > 100 ? 100 : positions.length;
        ILimitOrderManager.PositionTickRange[] memory leftoverPositions = new ILimitOrderManager.PositionTickRange[](positionsLength);
    ...
    // Helper function to construct position parameters
    function _constructPositionParams(
        ...
        // Get user position via getPositionBalances from lens
        ILimitOrderManager.PositionBalances memory posBalances = 
            lens.getPositionBalances(user, poolId, positionKey);
        
        // Create a position state
        ILimitOrderManager.PositionState memory posState = ILimitOrderManager.PositionState({
            feePerLiquidity: feePerLiquidity,
            totalLiquidity: totalLiquidity,
            isActive: isActive,
            isWaitingKeeper: isWaitingKeeper,
            currentNonce: currentNonce
        });
        
        // Create a user position
        ILimitOrderManager.UserPosition memory position = ILimitOrderManager.UserPosition({
            liquidity: 0, // We don't need this value since we're using getPositionBalances
            claimablePrincipal: toBalanceDelta(int128(uint128(posBalances.principal0)), int128(uint128(posBalances.principal1))),
            fees: toBalanceDelta(int128(uint128(posBalances.fees0)), int128(uint128(posBalances.fees1))),
            lastFeePerLiquidity: BalanceDelta.wrap(0) // We don't need this value
        });
    ...
    function test_batch_execute_scale_orders() public {
        deal(Currency.unwrap(currency0), address(this), 1000 ether);
        deal(Currency.unwrap(currency1), address(this), 1000 ether);
        ...
        // Set executablePositionsLimit to 50 to handle all orders
        ...
        ILimitOrderManager.CreateOrderResult[] memory results = limitOrderManager.createScaleOrders(
            true,                     // isToken0
            currentTick + 1,          // bottomTick - just above current tick
            currentTick + 3060,       // topTick
            50 ether,                 // totalAmount - 1 ether per order
            50,                       // totalOrders
            1e18,                     // sizeSkew - no skew
            poolKey                   // poolKey
        );
        ...
        ILimitOrderManager.PositionInfo[] memory positions = limitOrderManager.getUserPositions(address(this), poolId);
        uint256 initialPositions = positions.length;
        console.log("Total positions created:", initialPositions);
        assertEq(initialPositions, 50, "Not all orders were created");
        
        // Perform swap that crosses all price thresholds to execute all orders
        console.log("Executing swap to trigger all orders...");
        gasBefore = gasleft();
        
        swapRouter.swap(
            poolKey,
            IPoolManager.SwapParams({
                zeroForOne: false,                                    // false = swap token1 for token0
                amountSpecified: -100 ether,                          // input amount (negative means exact input)
                sqrtPriceLimitX96: TickMath.getSqrtPriceAtTick(currentTick + 3100) // Beyond all positions
            }),
            PoolSwapTest.TestSettings({
                takeClaims: false,
                settleUsingBurn: false
        ...
        uint256 swapGasUsed = gasBefore - gasleft();
        console.log("Gas used for executing orders via swap:", swapGasUsed);
        ...
        // Check how many positions are active
        uint256 activePositions = 0;
        for (uint256 i = 0; i < remainingPositions; i++) {
            (,, bool isActive,,) = limitOrderManager.positionState(poolId, positions[i].positionKey);
            if (isActive) activePositions++;
        }
        
        console.log("Active positions remaining:", activePositions);
        
        // Gas per executed order
        if (initialPositions > activePositions) {
            uint256 executedOrders = initialPositions - activePositions;
            console.log("Orders executed:", executedOrders);
            console.log("Gas per executed order:", swapGasUsed / executedOrders);
        }
    }
    function test_batch_claim_scale_orders() public {
        deal(Currency.unwrap(currency0), address(this), 1000 ether);
        deal(Currency.unwrap(currency1), address(this), 1000 ether);
        ...
        // Claim the executed orders in batches
        console.log("\n=== Claiming in batches ===");
        uint256 batchSize = 10;
        uint256 offset = 0;
        ...
        while (true) {
            uint256 claimedInBatch = limitOrderManager.claimBatchOrders(poolKey, offset, batchSize);
            if (claimedInBatch == 0) break;
            
            totalClaimed += claimedInBatch;
            batchCount++;
            
            console.log("Batch number:", batchCount);
            console.log("Orders claimed:", claimedInBatch);
            
            // If we claimed fewer than batch size, we're done
            if (claimedInBatch < batchSize) break;
            
            // Move to next batch
            offset += batchSize;
        }
        
        uint256 claimGasUsed = gasBefore - gasleft();
        console.log("Total orders claimed:", totalClaimed);
        console.log("Gas used for claiming orders:", claimGasUsed);
        if (totalClaimed > 0) {
            console.log("Gas per claimed order:", claimGasUsed / totalClaimed);
        }
    }
